...
しかもその制約は期待値ではなく経路ごとに課される（第\ref{sec:objective}章）。
...
\ref{part:catalog} & 写像の空間と制約 & $\Phi$ の全空間（27類型）、制約下の部分空間、その極限 \\
\ref{part:method} & 方法論 & 検証障害の分類と選択バイアスの扱い \\
...
第一に、本稿は\textbf{予測理論ではない}。第\ref{part:results}部で事前登録した予測は、
...
リスク引受の第三者 $R$ を想定する。
制度 $G$ は現金の相手方ではなく制約として扱う（第\ref{sec:layers}章）。
\end{definition}
...
上表の理論は、それぞれ異なる分野で異なる問いに答えるために発展した。
...
義務化せず残余請求権とし、
第\ref{sec:objective}章の倒産時刻 $\tau$ において強制的に清算する仕組みが
...
物理層はこの $C$ の性質を指す（第\ref{sec:layers}章）。
\end{remark}
...
\subsection{\texorpdfstring{$\CCC$}{CCC} は遅れ時間の和である}
