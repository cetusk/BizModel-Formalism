...
定額 $\pi = c$、従量 $\pi=p\,\delta$、二部 $\pi = F + p\,\delta$、成果連動 $\pi = h(y)$。
ここで $c$ は定額料金、$p$ は単価、$F$ は基本料金、$y$ は成果、
$h$ は成果を決済に変える関数である。
$y$ が観測できるかは自由度(4) に属し、第\ref{sec:info}章で扱う。
\end{enumerate}

\begin{remark}[$F$ と $c$ の区別]
\label{rem:dof2-symbols}
$F$ は基本料金であり、状態空間の $\sigma$ 加法族 $\mathcal{F}$ とは別の量である。
書体で区別する。

$c$ は定額料金を表す。引数を伴う $c(\delta)$ は限界費用（定義\ref{def:cost}）であり、
別の量である。以降、引数の有無で区別する。
\end{remark}
