...
\textbf{会員数 $n$ と平均利用 $\E[\delta]$ が独立に決まるとき}、
...
$m$ は第\ref{sec:growth}章で粗利率に用いるため、ここでは $n$ と書く。
\end{proof}
...
主体 $A$ と主体 $i$ が評価写像 $v$ を共有するとき、
$A$ の帳簿における $i$ への信用ポジションを $\kappa_i^{(A)}$ と書けば
\begin{equation}
\kappa_i^{(A)} = -\,\kappa_A^{(i)}
...
\begin{proof}
$A$ が $i$ に与えた価値は $i$ が $A$ から受け取った価値であり、
$A$ が $i$ から受け取った対価は $i$ が $A$ に与えた対価である。
したがって $D_i^{(A)}=P_A^{(i)}$、$P_i^{(A)}=D_A^{(i)}$ であり、
...
\begin{equation}
\sum_{A\in\mathcal{N}} W^{(A)}
= \kappa_{\text{対非法人}} + \sum_{A\in\mathcal{N}}\mathrm{Inv}^{(A)}
...
\begin{proof}
$\mathcal{N}$ の内部の各対 $(A,i)$ について式\eqref{eq:kappa-antisym}より
$\kappa_i^{(A)}+\kappa_A^{(i)}=0$ である。
$W=\sum_i\kappa_i+\mathrm{Inv}$ を $A$ について足すと内部の対は消え、
外部との対と在庫が残る。
\end{proof}
...
\label{prop:cash-net}
運転資本を $W(t)=\sum_i\kappa_i(t)+\mathrm{Inv}(t)$ とすると、式\eqref{eq:cash}は
\begin{equation}
M(t) = E(t) - W(t) - A(t)
\label{eq:cash-net}
\end{equation}
...
命題\ref{prop:cash-net}の運転資本 $W=\sum_i\kappa_i+\mathrm{Inv}$ を用いる。
...
式\eqref{eq:cash-net}に式\eqref{eq:ccc}を代入すると、現金残高が
\begin{equation}
...
式\eqref{eq:depletion}は、命題\ref{prop:pathwise}が記号で導入した倒産時刻に値を与える。
