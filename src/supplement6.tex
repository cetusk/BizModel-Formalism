...
本章が差し替えるのは第\ref{sec:info}章の $\mathcal{G}$ であり、
命題\ref{prop:implementable}の可測性条件を誰が担保するかを問う。
...
区分の差は第\ref{sec:dof}節の自由度(1)、すなわち時間関係に直接現れ、
式\eqref{eq:kappa}の $\kappa_i$ の符号を決める。
...
2-1 & 定額アクセス & ○ & ○ & $\pi=F$ が $\delta$ を参照しない \\
...
式\eqref{eq:kappa}の $\kappa_i$ について
...
が成立する。担保は自己の資産から重複なく供されるから $\sum_i B_i\leq E$ であり、
式\eqref{eq:protocol-collateral}を $i$ について足し合わせて次を得る。
...
第\ref{sec:obstacle-taxonomy}節の（v）とは異なる（注\ref{rem:agg-impossible}）。
...
第\ref{ch:protocol-emp}章で、これらの量を実際に観測する。
...
第\ref{sec:protocol-results}節の検定は演繹の含意を測るものであり、
前提そのものを検証しない。
