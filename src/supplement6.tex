...
族7は受益者と支払者が異なることを定義的性質とするが、
$\iota$ が観測できない以上、\textbf{両者が異なることを $\mathcal{P}$ は検証できない}。
族の定義そのものが $\mathcal{G}$ に載らない。
...
\begin{remark}[同時交換の消滅]
区分「外」では 1-1 が存在しない。同時性は $\delta$ と $\pi$ が
同一の状態遷移として実行される場合にのみ作れるためであり、
$\delta$ が外にあれば前受か後払のいずれかに退化する。
