\chapter{\texorpdfstring{$\Phi$}{Φ} の類型カタログ：七族二八類型}
\label{ch:catalog}
%============================================================
...
第\ref{sec:objective}節の割引因子の差を単独で商品化した形式である。
...
番号 & 類型 & $\kappa$ / 決済の性質 & 例 \\
\midrule
2-1 & 定額アクセス & 年払 $<0$ / 月末払 $>0$、$\pi=F$ & SaaS、会員制小売、施設利用型 \\
...
カタログを実証に用いるには、観測された $\Phi$ をどの類型に割り当てるかの規則が要る。
規則を事後に決めれば、結論に合うように割り当てられる。
以下に規則を定める。

割り当てに必要な情報は第\ref{sec:dof}節の自由度であり、
第\ref{ch:next}節で $\Phi$ ごとに記録すべき変数として一覧する量と一致する。

\textbf{族の判定条件は排他的でない。} したがって判定の順序を固定する。
...
\bottomrule
\end{tabularx}
\captionof{table}{族の判定順序。上から順に適用し、最初に該当した族を採る。}
\label{tab:family-assignment-order}
\end{center}

族が定まれば、族内の番号は自由度(1)（時間関係）、
自由度(2)（依存形式）、自由度(5)（権利と行使の分離）が指定する。

\begin{remark}[順序に依存する割り当てがある]
\label{rem:assignment-order-dependence}
表\ref{tab:family-assignment-order}の順序を変えれば割り当てが変わる $\Phi$ が存在する。

アプリ流通の手数料は、流通額に率を乗じる点で 6-1（取引額手数料）に当てはまり、
販売者の売上に連動する点で 5-2（レベニューシェア）にも当てはまる。
上の順序では族6が先に判定される。

\textbf{順序の選択に演繹的な根拠はない。} 主体の構成を決済形式より先に判定するという
方針によるものであり、規約である。
規約であるからこそ事前に固定しておく必要がある。
\end{remark}

\begin{remark}[複合契約の扱い]
\label{rem:composite-assignment}
第\ref{sec:composite}節のとおり、実際の契約は複数類型の合成であることが多い。
この場合はまず $\Phi$ を成分に分解し、各成分に表\ref{tab:family-assignment-order}を適用する。

分解の単位は、履行と決済の対応が一組閉じる範囲とする。
機器の販売と消耗品の供給は別の対応であるから二成分に分かれ、
それぞれ 1-1 と 2-2 に割り当てたうえで、合成として 4-1 と記す。
\end{remark}

\section{カタログの限界}

以上二八類型は網羅的ではない。欠落として検討した三つのうち、
二つは既存類型で記述できる。
...
カタログの二八類型はすべて企業が $\delta$ を提供する側を想定している。
...
さらに第\ref{sec:objective}節の倒産時刻 $\tau$ は吸収状態であったが、
労働者は支払不能により消滅しない。
...
命題\ref{prop:kappa-additive}より $\kappa$ は $\Phi$ の並置に対して加法的であるから、
複数の類型を並べた記述は $\kappa$ の水準で意味を持つ。
ただし注\ref{rem:decomposition-not-unique}のとおり\textbf{分解は一意でない}。
同じ $\kappa$ を与える類型の組は複数あるから、
類型は $\Phi$ の\textbf{生成系}であって基底ではない。
割り当ての手続きは注\ref{rem:composite-assignment}に定めた。
...
異符号の混合平均を観測することになる。
系\ref{cor:ccc-weighted}より、その平均は売上速度による加重平均であり、
族ごとの $\CCC$ の単純平均ではない。
