...
取得できた「棚卸資産」の対象は10業種・11規模階層・2015--2024年度に限られる。
本章の時系列が用いる45業種を覆わないため、
以下は\textbf{本章の推定を置き換えるものではなく、感度の確認である}。
業種・規模・年度で突き合わせた1,100件を用いる。

\subsection{水準は約二倍の過小である}

$\mathrm{DIO}$ の倍率は中央値で 1.89 倍、差は中央値 4.1 日・平均 8.4 日である。
...
\subsection{差分はほぼ影響を受けない}

$\Delta\CCC$ について両定義を比較すると、990観測で相関 0.976、
符号の一致は 91.0\%、標準偏差は 10.09 日から 10.40 日である。

\subsection{弾力性は定義の是正で改善する}

共通標本（集約系列を除く8業種、733観測）で式\eqref{eq:wc-elasticity}を推定する。
...
\begin{tabular}{@{}lrrr@{}}
\toprule
$\mathrm{DIO}$ の定義 & $\beta$ & $R^2$ & 分母効果 $1-\beta$ \\
\midrule
製品又は商品のみ & 0.900 & 0.092 & 0.100 \\
棚卸資産（総額） & \textbf{0.960} & \textbf{0.213} & \textbf{0.040} \\
\bottomrule
\end{tabular}
\captionof{table}{棚卸資産の範囲による弾力性 $\beta$ の差（共通標本733観測）。}
\label{tab:inventory-elasticity}
\end{center}

棚卸資産を総額で取ると $\beta$ は 1 に近づき、決定係数は倍以上になる。
...
$\beta$ は 0.894 と 0.787 となり、分母効果が\emph{増える}という逆の結論が出る。
...
第\ref{sec:denominator-effect}節の結論は維持され、方向としては強まる。
...
主張 & 判定 \\
\midrule
水準の絶対値は解釈できない & 支持。$\CCC$ は業種により3割以上動く \\
差分と相関を扱う分析には影響が小さい & 支持。$\Delta\CCC$ の相関 0.976 \\
分母効果は支配的でない & 支持。是正すると寄与はさらに小さくなる \\
\bottomrule
\end{tabularx}
\captionof{table}{棚卸資産の範囲に関する三つの主張の判定。}
\label{tab:inventory-verdict}
\end{center}

ただし対象は10業種にとどまる。
本章の45業種の推定を是正するには、当該項目を全業種について取得する必要がある。
