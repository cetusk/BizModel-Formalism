%============================================================
\chapter{法人企業統計による実証：\texorpdfstring{$\kappa$}{κ} と \texorpdfstring{$\CCC$}{CCC} の横断}
\label{part:industry}
%============================================================

\section{位置づけ}

第\ref{part:theory}部で定義した $\kappa$ と $\CCC$ を、
法人企業統計の業種別・規模別の集計に当てる。二つの横断を行う。

\textbf{規模の横断}では、信用の向きが規模階層とどう対応するかを見る。
第\ref{sec:layers}節は信用層を交渉力の写像として書いており、
そうであれば小規模ほど与信の出し手になるはずである。

\textbf{時間の横断}では、$\CCC$ の変化が業界の状態、
すなわち\textbf{需要が逼迫しているかどうか}の指標になるかを見る。
ただし「業界が熱い」は一次元ではなく、$\CCC$ が触れるのはその一方の軸にすぎない。
これを次節で確定する。

結論を先に述べると、\textbf{どちらの含意も外れた}。信用の向きは符号が逆であり、
先行性は完全に消滅した。

\section{ホットとコールドは一次元ではない}

測定の前に、判定対象を確定する。
式\eqref{eq:gstar}の $g^\star=(m+d-I/r)/\CCC$ は、
収益性を表す分子と資金繰りを表す分母という別の二軸から成っていた。
...
\bottomrule
\end{tabularx}
\captionof{table}{ホット・コールドを判定する二軸と測定可能性。}
...
$\kappa_S$ & 式\eqref{eq:kappa} & \textminus DPO。DPO は 21--44 日（規模別） \\
...
第\ref{sec:inventory-scope}節では、これに棚卸資産（当期末）を加えた突合を行う。
...
\begin{quote}
\textbf{登録した含意}：小規模階層ほど $\mathrm{DSO}-\mathrm{DPO}$ が大きい。
すなわち小規模ほど与信の出し手になっている。
\end{quote}

根拠は第\ref{sec:layers}節の「信用層は交渉力の写像である」という主張であった。
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
規模（資本金） & DSO & DIO & DPO & DSO\textminus DPO \\
\midrule
...
\bottomrule
\end{tabular}
\captionof{table}{資本金階層別の DSO・DIO・DPO と DSO\textminus DPO。}
...
正味の与信の出し手、すなわち $\sum_i\kappa_i>0$ である主体は
$E > A$ を満たす。
\end{proposition}

\begin{proof}
命題\ref{prop:fundamental}の基本不等式より $W\leq E-A$ である。
$W=\sum_i\kappa_i+\mathrm{Inv}$ で $\mathrm{Inv}\geq 0$ であるから、
$\sum_i\kappa_i>0$ なら $W>0$、したがって $E-A>0$ を得る。
\end{proof}

第\ref{part:solo}章の $E\approx 0$ では逆に $\kappa<0$ が要求された（系\ref{cor:zero-equity}）。
...
全産業 & 27 & 14 & +13 \\
製造業 & 26 & 24 & +2 \\
職業紹介・労働者派遣業 & 24 & 25 & \textminus 1 \\
小売業 & \textminus 16 & 6 & \textbf{\textminus 22} \\
\bottomrule
\end{tabular}
\captionof{table}{業種別の DSO\textminus DPO（資本金10億円以上と1千万円未満の比較）。小売業のみ符号が反転する。}
\label{tab:dso-dpo-by-industry}
\end{center}
\begin{center}\footnotesize DSO\textminus DPO、2024年度、単位は日\end{center}
...
\begin{quote}
\textbf{登録した含意}：$\CCC$ の変化率は売上高変化率に対して\emph{先行または同期}する。
...
同期 $k=0$ & \textminus 0.159 & \textminus 0.228 & 69\% & $p=0.008$ \\
遅行 $k=+1$ & 0.072 & 0.118 & 31\% & --- \\
遅行 $k=+2$ & 0.085 & 0.035 & 44\% & --- \\
\bottomrule
\end{tabular}
\captionof{table}{$\Delta\CCC(t)$ と売上高成長率 $g(t+k)$ のラグ別相関（45業種集計）。}
...
\bottomrule
\end{tabularx}
\captionof{table}{先行性に関する三つの主張の判定。}
...
負に強い業種 & $r$ & 正に強い業種 & $r$ \\
\midrule
パルプ・紙・紙加工品製造業 & \textminus 0.78 & 農業、林業 & +0.80 \\
金属製品製造業 & \textminus 0.67 & その他の輸送用機械器具製造業 & +0.69 \\
情報通信業 & \textminus 0.58 & 宿泊業、飲食サービス業 & +0.54 \\
生産用機械器具製造業 & \textminus 0.53 & 農林水産業 & +0.52 \\
\bottomrule
\end{tabular}
\captionof{table}{同期相関 $|r|>0.4$ の業種（負に強い4業種・正に強い4業種）。}
...
図\ref{fig:ccc-trend}のとおり、全産業の $\CCC$ は2000年度の24.9日から2024年度の37.2日へ、
25年間でほぼ単調に増加している。正味与信ポジションも9.8日から18.5日へ倍増した。
\textbf{日本の企業部門全体が、運転資本をより多く抱える方向へ動いている。}

\begin{remark}[集計値が測っているもの]
系\ref{cor:kappa-nets-out}より、法人部門の内部で閉じた信用ポジションは
集計で相殺されて消える。したがってここでの $\CCC$ は企業間信用の指標ではなく、
対非法人ポジションと在庫の合計を売上で割った量である。
実際、表\ref{tab:dso-dpo-by-size}の全産業で DSO $-$ DPO は 13 日であり、
$\CCC=37.2$ 日との差の大部分を在庫が占める。
\textbf{集計の $\CCC$ の主成分は在庫である。}
個別企業に現れる負の $\CCC$ が集計に現れないのも同じ理由による。
\end{remark}
...
\bottomrule
\end{tabular}
\captionof{table}{$\CCC$、$m$、$m/\CCC$ の2000年度から2024年度への変化。}
...
$\CCC$ の上昇のみを観測して結論を導けば、比の一方だけを見たことになる。
...
\bottomrule
\end{tabular}
\captionof{table}{$\CCC$ が長い業種・短い業種（各上位6業種）。}
...
\begin{quote}
\textbf{登録した含意}：$\CCC$ の上昇は、業種内効果より構成効果の寄与が大きい。
...
系\ref{cor:ccc-weighted}より、全体の $\CCC$ は業種別の売上加重平均
$\CCC=\sum_k w_k\CCC_k$ である。この差分は二項に分かれる。
\begin{equation}
...
\end{equation}
$\bar{\cdot}$ を二期の中点とすれば式\eqref{eq:shift-share}は\textbf{残差なしで厳密に成立する}。
実際、$\bar w=(w_1+w_0)/2$、$\bar\CCC=(\CCC_1+\CCC_0)/2$ を代入すると
右辺は $\sum_k(w_{k1}\CCC_{k1}-w_{k0}\CCC_{k0})$ に等しい。
これは測定のために導入した近似ではなく、
$\CCC$ が比であること（系\ref{cor:ccc-weighted}）の帰結である。
...
\bottomrule
\end{tabular}
\captionof{table}{$\Delta\CCC$ のシフト・シェア分解（業種内効果と構成効果）。}
\label{tab:shift-share-decomposition}
\end{center}

\textbf{この含意は棄却される。} 業種内効果が8割から9割8分を占める。
...
期間 & DSO & DIO & DPO & 主因 \\
\midrule
2000$\to$2024 & \textminus 0.93 & +3.49 & \textbf{+6.90} & DPO の短縮 \\
2010$\to$2024 & \textbf{+5.26} & +4.02 & +0.62 & DSO の伸長 \\
\bottomrule
\end{tabular}
\captionof{table}{業種内効果の DSO・DIO・DPO への成分分解。}
...
\bottomrule
\end{tabular}
\captionof{table}{系\ref{cor:ccc-change}による契約の変化と運用の変化の分解。}
...
\bottomrule
\end{tabular}
\captionof{table}{受取手形・支払手形の推移（2000・2010・2024年度）。}
...
\textbf{$\beta$ の理論値は $1$ である。}
式\eqref{eq:ccc}より $W=\CCC\cdot r$ であり、
$\CCC$ が一定なら $\Delta\ln W=\Delta\ln r$ となる。したがって
\begin{equation}
\Delta\ln\CCC = (\beta-1)\,\Delta\ln r
\label{eq:beta-ccc}
\end{equation}
であり、$\beta$ の $1$ からの乖離は $\CCC$ の売上弾力性そのものである。
系\ref{cor:ccc-change}の式\eqref{eq:ccc-change}はこの動きを契約の変化と運用の変化に分解している。

\begin{quote}
\textbf{登録した含意}：$\Delta\ln W$ と $\Delta\ln r$ は正の相関を持つ。
すなわち売上が減れば運転資本も減る。
\end{quote}
...
\bottomrule
\end{tabularx}
\captionof{table}{$\Delta\ln W$ と $\Delta\ln r$ の弾力性推定（式\eqref{eq:wc-elasticity}）。}
\label{tab:wc-elasticity-result}
\end{center}

\textbf{この含意は支持される。} 運転資本は売上にほぼ比例して動き、
分母効果に帰せられるのは17\%程度である。
第\ref{sec:e-verification}節の同期負相関は、大部分が需要側の情報を含む。

式\eqref{eq:beta-ccc}に $\beta=0.830$ を入れると、$\CCC$ の売上弾力性は $-0.170$ である。
すなわち\textbf{売上が落ちる局面で $\CCC$ は上がる}。
注\ref{rem:ccc-amplifier}の増幅係数はこのとき大きくなるから、
需要の下振れが現金に伝わる度合いは不況期に強まる。
$1-\beta$ を「分母効果」と呼ぶのは計算上の呼称であって、
$\CCC$ が動いていないという意味ではない。
...
\begin{quote}
\textbf{登録した含意（その1）}：$\Delta\ln r(t-1)$ の係数 $\beta_1$ は、
その業種の $\CCC$ が長いほど大きい。

\textbf{登録した含意（その2）}：ラグを含めた総弾力性 $\beta_0+\beta_1$ は
...
\toprule
$\beta_0$（同時点） & 0.836 \\
$\beta_1$（1期ラグ） & \textminus 0.015 \\
総弾力性 & 0.821 \\
$R^2$ & 0.081 \\
\bottomrule
\end{tabular}
\captionof{table}{分布ラグ模型の推定結果。}
...
\section{棚卸資産の範囲が結論に与える影響}
\label{sec:inventory-scope}

本章の $\mathrm{DIO}$ は「製品又は商品」のみを用いており、
仕掛品と原材料を含まない。法人企業統計は「棚卸資産（当期末）」を別項目として持つため、
両者の差が過小計上の大きさになる。本節はその大きさを測る。

\textbf{本節は含意の検定ではない。} 同一のデータについて測定の定義を差し替え、
結論がどれだけ動くかを確認するものである。

\subsection{二つの断面}

「棚卸資産（当期末）」は二通りの切り口で取得した。
\textbf{両者は識別している変動が異なる}ので、区別して読む必要がある。

\begin{center}
\begin{tabularx}{\textwidth}{@{}lXr@{}}
\toprule
断面 & 覆う範囲と、$\beta$ を識別する変動 & 観測 \\
\midrule
規模断面 & 10業種 $\times$ 11規模階層 $\times$ 2015--2024年度。$\beta$ は\textbf{規模階層のあいだ}の変動で識別される & 1,100 \\
業種断面 & 62業種 $\times$ 全規模 $\times$ 2000--2024年度。$\beta$ は\textbf{業種と年度のあいだ}の変動で識別される & 1,364 \\
\bottomrule
\end{tabularx}
\captionof{table}{棚卸資産の取得の二断面。}
\label{tab:inventory-cuts}
\end{center}

本章の推定（第\ref{sec:denominator-effect}節）は45業種946観測であり、
\textbf{業種断面と同じ設計である}。したがって本章を是正する数値は業種断面のほうであり、
規模断面は別の問いに答えている。以下、断りのない数値は業種断面による。

\subsection{水準は約二倍の過小である}

$\mathrm{DIO}$ の倍率は中央値で 1.99 倍、
$\CCC$ の中央値は 28.0 日から 43.9 日に増える。
全産業（除く金融保険業）では2000年度が 24.9 日から 38.2 日、
2024年度が 37.2 日から 53.5 日である。

規模断面では倍率の中央値 1.89 倍、差は中央値 4.1 日・平均 8.4 日であり、
同程度の大きさが出る。
...
\midrule
製造業 & 19.2 & 50.4 & 41.6 & \textbf{72.8} \\
建設業 & 9.7 & 33.8 & 39.8 & \textbf{63.9} \\
全産業（除く金融保険業） & 18.7 & 35.0 & 37.2 & \textbf{53.5} \\
...
\bottomrule
\end{tabular}
\captionof{table}{棚卸資産の範囲による $\mathrm{DIO}$ と $\CCC$ の差（全規模、2024年度、単位は日）。}
\label{tab:inventory-scope}
\end{center}

差は業種によって桁が異なる。製造業と建設業で $\CCC$ が3割以上増える一方、
小売業と職業紹介・労働者派遣業ではほとんど変わらない。
\textbf{仕掛品を持つ業種でのみ乖離が生じる}という当然の構造だが、
第\ref{sec:ccc-extremes-by-industry}節の業種別水準の順序が変わる大きさである。

\subsection{差分は影響が小さいが、無視できるほどではない}

$\Delta\CCC$ について両定義を比較すると、業種断面の1,302観測で相関 0.882、
符号の一致は 86.8\% である。
規模断面（990観測）では相関 0.976、符号一致 91.0\% であり、
\textbf{業種をまたぐほうが一致は落ちる}。
業種によって仕掛品の比重が違うため、業種間の変動を含めると差が出る。

\subsection{弾力性は、定義の是正でわずかに悪化する}

共通標本で式\eqref{eq:wc-elasticity}を推定する。
定義ごとに $W>0$ となる観測が異なるため、
\textbf{両定義とも $W>0$ である観測に限って比較する}。

\begin{center}
\begin{tabular}{@{}llrrr@{}}
\toprule
断面 & $\mathrm{DIO}$ の定義 & $\beta$ & $R^2$ & 分母効果 $1-\beta$ \\
\midrule
業種断面 & 製品又は商品のみ & 0.840 & 0.073 & 0.160 \\
$n=955$ & 棚卸資産（総額） & \textbf{0.828} & \textbf{0.166} & \textbf{0.172} \\
\midrule
...
\bottomrule
\end{tabular}
\captionof{table}{棚卸資産の範囲による弾力性 $\beta$ の差。共通標本による。}
\label{tab:inventory-elasticity}
\end{center}

どちらの断面でも、棚卸資産を総額で取ると決定係数は倍以上になる。
\textbf{運転資本は、仕掛品と原材料を含めたほうが売上によく比例する。}

\textbf{しかし分母効果の動く向きは断面で逆になる。}
規模断面では 0.100 から 0.040 へ小さくなるのに対し、
業種断面では 0.160 から 0.172 へわずかに大きくなる。

\begin{remark}[断面が変われば向きが変わる]
\label{rem:inventory-cut}
規模断面では是正が分母効果を小さくし、業種断面では大きくする。
\textbf{両者は誤差ではなく、識別している変動が違う}（表\ref{tab:inventory-cuts}）。
規模階層のあいだでは、大企業ほど仕掛品を多く抱えるため
総額で取ると $W$ が売上によく比例する。
業種のあいだでは、仕掛品の比重そのものが業種の性格として散らばるため、
総額で取っても比例は改善しない。

本章の推定は業種と年度の変動で識別されているので、
\textbf{本章に当てはまるのは業種断面のほうである}。
一方の断面で測った向きを他方に持ち込むことはできない。
\textbf{標本の覆う範囲は、結論の及ぶ範囲と同じではない。}
\end{remark}

\begin{remark}[標本を揃えないと符号が逆に出る]
\label{rem:common-sample}
定義ごとに $W>0$ の観測が異なるため、標本を揃えずに推定すると
規模断面の $\beta$ は 0.894 と 0.787 となり、
分母効果が\emph{増える}という逆の結論が出る。
共通標本に限れば 0.900 と 0.960 である。
\textbf{定義の差を測るときは、定義の変更が標本の構成を変えることに注意を要する。}
\end{remark}

\subsection{判定}

第\ref{sec:denominator-effect}節の結論は維持されるが、
\textbf{強まるのではなく、そのまま維持されるにとどまる}。
...
主張 & 判定 \\
\midrule
水準の絶対値は解釈できない & 支持。$\CCC$ の中央値が 28.0 日から 43.9 日に動く \\
差分と相関を扱う分析には影響が小さい & 支持。ただし $\Delta\CCC$ の相関は 0.882 で、規模断面の 0.976 より低い \\
分母効果は支配的でない & 支持。ただし是正すると 0.160 から 0.172 へわずかに\emph{増える} \\
\bottomrule
\end{tabularx}
\captionof{table}{棚卸資産の範囲に関する三つの主張の判定。業種断面による。}
\label{tab:inventory-verdict}
\end{center}

三つとも支持されるが、いずれも判定は強くない。
$1-\beta$ が 0.172 であることは、運転資本の増加のうち
売上に比例しない部分が 17\% あるということであり、
支配的ではないが無視もできない。
...
\bottomrule
\end{tabular}
\captionof{table}{60歳以上プランの割引率（施設カテゴリ別、利用量段階別）。}
...
平日昼間という時間帯の限定により $\operatorname{supp}\delta_i$ が
...
第\ref{sec:retiree}節に関連して、次の含意を事前に固定した。

\begin{quote}
\textbf{登録した含意}：高齢層ほど定額契約より都度払いを選ぶ。
...
\item \textbf{高齢層の契約選択を検証していない。} 第\ref{sec:cap-observation}節のとおり、
消費者側の選択データが得られていない。供給側の価格設計は対抗仮説と整合するが、
間接証拠にとどまる。

\item \textbf{棚卸資産が「製品又は商品」のみである。} 仕掛品と原材料を含めていないため、
$\mathrm{DIO}$ は過小であり、$\CCC$ の水準は実際より短く出ている。
第\ref{sec:inventory-scope}節で10業種について差を測り、
差分と相関を扱う分析には影響が小さいことを確認したが、
\textbf{45業種の推定を是正するには当該項目を全業種について取得する必要がある}。
\end{enumerate}

\section{二つの含意が両方外れたことについて}
\label{sec:two-rejections}

信用の向きは符号が逆、先行性は消滅した。登録した含意は両方とも外れている。

第\ref{part:results}部で事前登録の効果を記録したが、本章ではそれがより明瞭に働いた。
含意を先に固定していなければ、同期の負相関を
「$\CCC$ は景気指標として機能する」と読み、
大企業の与信ポジションを「大企業が取引先を支えている」と解釈していた可能性が高い。
いずれも後付けの物語である。

なお、含意が外れたことは枠組みの棄却ではない。
