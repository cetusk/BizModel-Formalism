...
\textbf{規模の横断}では、信用の向きが規模階層とどう対応するかを見る（予測F）。
第\ref{sec:layers}章は信用層を交渉力の写像として書いており、
そうであれば小規模ほど与信の出し手になるはずである。

\textbf{時間の横断}では、$\CCC$ の変化が業界の状態、
すなわち\textbf{需要が逼迫しているかどうか}の指標になるかを見る（予測E）。
ただし「業界が熱い」は一次元ではなく、$\CCC$ が触れるのはその一方の軸にすぎない。
これを次節で確定する。

結論を先に述べると、\textbf{両方とも棄却された}。予測Fは符号が逆であり、
...
\bottomrule
\end{tabularx}
\captionof{table}{予測Eに関する三つの主張の判定。}
...
\bottomrule
\end{tabular}
\captionof{table}{分布ラグ模型の推定結果（予測O・Pの検定）。}
...
本章の推定（第\ref{sec:denominator-effect}節、予測N）は45業種946観測であり、
...
三つとも支持されるが、いずれも第14段階に10業種で測ったときより判定が弱い。
\textbf{標本を広げると主張は弱まる方向に動いた。}
