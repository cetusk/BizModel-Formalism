...
本稿では17の予測を事前に文章として固定した。
支持されたのは6件（支持2件、部分的支持4件）、棄却が7件、検証不能が4件である。
...
\bottomrule
\end{tabularx}
\captionof{table}{事前登録した17予測の対象・内容・判定結果。}
\label{tab:predictions-16}
\end{center}

Q・R・S は労働者視点の検討過程で登録したが、
第\ref{sec:worker-predictions}節のとおり検証に着手していない。
Z は第\ref{sec:b1-proxy}節で、
$\Phi$1--$\Phi$4 は第\ref{sec:protocol-predictions}節で登録したが、
いずれも本稿が用いていない標本を要するため検定していない。
...
\item \textbf{手続き上の記録による観測。}
第\ref{sec:protocol-observability}節のとおり、
第\ref{ch:protocol}章の区分「内」では $M(t)$、$\kappa_j$、$\dbar-\delta$ が
個別に公開されている。本稿が個社データを要するとして測定を断念した量である。

第\ref{sec:protocol-predictions}節で予測$\Phi$1--$\Phi$4 を登録した。
$\Phi$1 は第\ref{part:unmanned}章の命題、$\Phi$2 は第\ref{ch:solo}章の経路制約、
$\Phi$3 と $\Phi$4 は第\ref{ch:protocol}章の系と命題に対応する。
\textbf{第\ref{part:catalog}部の演繹に対する初めての検定になる。}
...
手続きが強制する量を観測しても検証にならない点にも注意を要する
（表\ref{tab:protocol-testability}）。
...
四成分のうち三つに公開データが存在する（第\ref{ch:platformfee}章）。
$\lambda$ の測定から停止事象への接続を自ら構築する必要があり、そこに作業量が集中する。

\item \textbf{予測Zの検定。}
予測Yは第\ref{sec:predY-result}節で検定し、部分的に支持された。
東京都を除いた標本でも施設介護と保育士は正、看護師はゼロ近傍であり、
この区分は集計の規則によらない。ただし有意なのは施設介護のみである。

残るのは予測Zである。第\ref{sec:predY-result}節の標本は全国を覆っており、
\textbf{欠けているのは返戻金制度の強度のみである}。
掲載形式が事業者ごとに異なるため一件ずつ人が読む作業になるが、
適正認定事業者60--70者に限れば実行可能な規模である。

予測Zが成立すれば、符号が一意に定まらないとした予測Bについて
B1とB2を分離する経路が得られる。
ただし代理として用いうるだけであり、
投入の希薄化と選別の緩みは区別できない（注\ref{rem:b1-proxy-assumption}）。
...
\toprule
 & 項目 & 分類 & 実行可能性 & 波及 \\
\midrule
1 & (13) 予測$\Phi$1--$\Phi$4 の検定 & 集計の不能 & 高 & 大 \\
2 & (18) 予測Zの検定 & 障害なし & 中 & 大 \\
3 & (17) $\lambda_{\rm novel}$ の成分分解 & 障害なし & 中 & 中 \\
4 & (3) R2 の $H$ と $\beta$ & (ii) & 中 & 中 \\
5 & (5) $\Delta\ln W$ の変動要因 & (iii) & 中 & 中 \\
6 & (11) 個人事業主の $\kappa$ & 集計の選択 & 中 & 大 \\
7 & (19) 棚卸資産の範囲（規模との交差） & 集計の選択 & 低 & 小 \\
\bottomrule
\end{tabularx}
...
\toprule
 & 項目 & 分類 & 前進しない理由 \\
\midrule
8 & (6) $\phi_{\rm cog}$ & (iv) & 個票と外生ショックに到達しない \\
9 & (1) 役員報酬 & (i) & 労働時間の統計が存在しない \\
10 & (2) 可測性仮説 & (ii)＋(vi) & 説明変数が動いていない \\
11 & (7) 余剰の取りやすさ & (iv) & 仲介者は業種の属性でない \\
12 & (4) 売掛金伸長の解釈 & (iii) & 電子記録債権の残高が調査項目にない \\
13 & (8) 年齢効果の曲率 & (iv) & 企業年齢を持つ標本がない \\
14 & (14) 無人化の程度 & --- & 測度が定義されない \\
15 & (15) $\phi$ の成分分離 & --- & $p^{\ast}$ が反実仮想 \\
16 & (10) 集約の社会的最適性 & --- & 対象設定の外 \\
17 & (9) 容量制約 & --- & 同上 \\
18 & (12) 請求権の流通 & --- & 定義域の拡張を要する \\
19 & (16) 第三者の除去の程度 & --- & 同上 \\
\bottomrule
\end{tabularx}
...
\textbf{障害を持たない項目は三件あり、いずれも公表データまたは
既に確立した取得手続きで着手できる。}
(19)は10業種について実施済みで、残るのは全業種への拡張である
（第\ref{sec:inventory-scope}節）。
(18)は標本を取得済みであり、欠けているのは返戻金制度の強度のみである。
表\ref{tab:unresolved-blocked}の12項目とは、要する作業の種類が異なる。
