...
\caption{理論として導出した主張とその状態。}\label{tab:theory-achievements}\\
\toprule
主張 & 内容 & 状態 \\
\midrule
\endfirsthead
...
$\CCC\cdot r\leq E-A$ & 現金制約は運転資本の制約である & 基本不等式（命題\ref{prop:fundamental}） \\
$r^{\max}=(E-A)/\CCC$ & 規模の上限。$g^\star$ とは別の制約 & 導出済（系\ref{cor:rmax}） \\
$g^\star = (m+d-I/r)/\CCC$ & 自己金融可能成長率。$\dot M\geq 0$ の境界 & 導出済（系\ref{cor:gstar}） \\
現金の枯渇時刻 & $g>g^\star$ が効くまでの時間 & 閉形式（命題\ref{prop:depletion}、例\ref{ex:depletion}） \\
$\CCC=\sum\tau_i+\tau_{\mathrm{inv}}$ & $\CCC$ は遅れ時間の和 & 導出済（命題\ref{prop:ccc-structure}） \\
$\tau_i$ の完全代替 & 回収の短縮と支払の延長は等価 & 導出済（命題\ref{prop:tau-substitution}） \\
$\kappa$ の反対称性 & 企業間信用は集計で消える & 導出済（命題\ref{prop:kappa-antisym}） \\
尖頭と平均の差 & 必要資本は $\CCC+T_b/2$ で決まる & 導出済（命題\ref{prop:peak}） \\
認知余剰の現金中立性 & 認識時点では現金が動かない & 導出済（命題\ref{prop:cog-cash-neutral}） \\
認知余剰の蓄積上限 & 取引頻度が規模の上限を決める & 導出済（系\ref{cor:cog-bounded}） \\
拘束の入れ替わり & $\CCC^{\dagger}$ を境に現金から容量へ & 導出済（命題\ref{prop:binding-switch}） \\
可用性と粒度 & 容量が余っても実行できない履行 & 導出済（命題\ref{prop:granularity}） \\
容量制約下の会員制 & 低稼働が実行可能性の条件 & 導出済（命題\ref{prop:capacity}） \\
一人事業の配分 & 受託は制約が要求する最小限 & 導出済（命題\ref{prop:allocation}） \\
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.42\textwidth}>{\raggedright\arraybackslash}p{0.26\textwidth}@{}}
\caption{実証で判定した主張とその状態。棄却・未検証を含む。}\label{tab:empirical-judgments}\\
\toprule
主張 & 内容 & 状態 \\
\midrule
\endfirsthead
...
分母効果の寄与 & $\CCC=W/r$ の機械的変動 & 17\%程度。$\CCC$ の売上弾力性は $-0.17$ \\
...
規模と与信の向き & 小規模ほど与信の出し手 & \textbf{棄却、符号が逆}（命題\ref{prop:credit-needs-equity}） \\
...
経路制約の吸収性 & $M(t)$ は途絶に先行して減るか & \textbf{支持}（第\ref{sec:protocol-results}節） \\
依存と無人継続期間 & 依存が少ないほど改修が遅いか & \textbf{棄却}。複製の集中が結果を作る \\
族7の実装可能性 & 残るのは 7-4 か & \textbf{棄却}。稀である点のみ当たる \\
\midrule
無人継続期間 & $1/\lambda_{\rm novel}$ の水準 & 20 年より短い（注\ref{rem:lambda-upper}） \\
手続きの継続期間 & 全数での測定 & 停止 8.3\%、中央値 1.62 年（第\ref{sec:protocol-byproducts}節） \\
\end{longtable}

記述の道具としての枠組みは機能した。予測の道具としては機能しなかった。
この二つは別の性質であり、本稿は前者のみを主張する。

\section{含意を事前に固定したこと}
\label{sec:prereg-worked}

本稿は理論から導いた含意を、観測にあたる前に文章として固定した。
支持・棄却の別を後から選べないようにするためである。
登録は22件で、支持6件、棄却11件、検証不能5件であった。
登録の文言と判定の一覧は付録\ref{app:prereg}にある。

\textbf{棄却の多さは方法の失敗ではない。}
固定していなければ、観測されたどんな事実にも事後の説明を与えられた。
付録\ref{app:prereg}に、実際に後付けの解釈を採りかけた四箇所を記録した。

\section{未解決の問題}

未解決の問題を、第\ref{sec:obstacle-taxonomy}節の六分類に従って整理する
...
検証には高齢層の契約選択の観測が要るが、年齢別の契約形態選択分布が得られていない。
...
迂回路として $\delta^{*}=F/p$ を構成したが、二択メニューにのみ適用でき、
...
リスク成分については含意を登録したが（第\ref{sec:pred-J}節）、
$\rho$ の推定に要するデータが仲介者の内部にあり、
かつ $\rho$ の高い業種は審査で排除されるため検証できない。

\item \textbf{年齢効果の曲率。}
命題\ref{prop:apc}により三係数の水準は識別されないが、
二階差分すなわち曲率は識別される（注\ref{rem:apc-identified}）。

\textbf{これを追うには企業年齢を持つ標本が要る。}
本稿が用いた法人企業統計は業種・規模・年度の三軸であり、設立年を含まない。
注\ref{rem:apc-in-this-work}のとおり $\CCC$ の時系列には年齢効果が潜在しているが、
分離する手段を持たない。
\end{enumerate}
...
\item \textbf{手続き上の記録による観測。検定は行った。}
第\ref{ch:protocol-emp}章のとおり検定した。
第\ref{part:catalog}部の演繹に対する初めての検定であり、支持されたのは $\Phi$2 のみである。

\textbf{残るのは主体単位の量である。}
観測できるのは識別子単位であり、
一主体が複数の識別子を持つため主体単位の $\kappa_i$ を復元できない
（注\ref{rem:additivity}）。障害は集計の不在ではなく\textbf{集計の不能}であり、
第\ref{sec:sixth-obstacle}節で追加した二形式のいずれとも異なる。
検定を経て、この乖離には第二の形式があることが分かった。
同一の実装を指す識別子の集中である（注\ref{rem:clone-concentration}）。

また検証できたのは演繹の正しさであって、
その演繹が現実の一人事業に当てはまるかではない（注\ref{rem:protocol-population}）。

前項(12)と同じ箇所、すなわち添字の設定に由来する制約である。

\item \textbf{無人化の程度を表す量。}
「どの程度無人か」を比として表すことができない。
履行の側で測れば評価写像 $v$ が価値を測るにとどまり、
応答の側で測れば $\Omega\setminus\hat\Omega$ に測度が定義されない
（注\ref{rem:automation-rate-illposed}）。
障害は単位の不在ではなく測度の不在であり、定義の精密化では解消しない。

\item \textbf{$\phi_{\rm prod}$ と $\phi_{\rm barg}$ の分離。}
定義\ref{def:phi-prod}は支配力が働かない場合の価格 $p^{\ast}$ を要するが、
これは反実仮想であり観測されない（注\ref{rem:counterfactual-price}）。
第\ref{sec:platform-fee}章で数値的に分離できたのは、
同一プラットフォーム内で機能のみが異なる比較が得られた例外的な事例による。
一般の対象については $p^{\ast}$ を特定する追加の仮定が要る。

\item \textbf{第三者の除去の程度。}
第\ref{ch:protocol}章は決済の記録と執行を手続きが完全に担う場合と
全く担わない場合のみを扱い、中間を扱わない（注\ref{rem:oracle}）。
外の事実を状態に書き込む主体の構成には幅があり、
実在する仕組みの多くは中間にある。
\end{enumerate}

\subsection{障害を持たないもの}

以下は第\ref{sec:obstacle-taxonomy}節のいずれの障害にも該当しない。
取得経路が確立しているか、既に取得したデータで実施できる。
着手していないだけである。

\begin{enumerate}[label=(\arabic*),start=17]
\item \textbf{$\lambda_{\rm novel}$ の成分分解。}
第\ref{sec:protocol-byproducts}節で応答の率を測った。
改修が毎年 0.048、停止が 0.030 であり、
命題\ref{prop:unmanned-period}の $1/\lambda_{\rm novel}$ は 20 年より短い。

\textbf{残るのは成分への分解である。}
測れたのは応答の合計であって、脆弱性・依存・制度・需要のいずれに由来するかを分けていない。
分けるには改修の内容を読む必要があり、
検証済みソースを持つものに限られるため標本が偏る。

\item \textbf{棚卸資産の範囲の是正。}
第\ref{part:industry}章は「製品又は商品」のみを用いており、
仕掛品と原材料を含めていないため $\mathrm{DIO}$ が過小である。

\textbf{これは62業種について実施した}（第\ref{sec:inventory-scope}節）。
$\CCC$ の中央値は 28.0 日から 43.9 日に動くが、
$\Delta\CCC$ の相関は 0.882、分母効果は 0.160 から 0.172 に留まる。
本章の結論は維持される。

残るのは規模階層との交差である。
「棚卸資産（当期末）」は62業種では全規模でしか取得できず、
10業種でしか規模階層別に取得できない。
\textbf{業種と規模の両方で分けた断面は、e-Stat の10万セル制限を超える。}
分割して取得すれば可能だが、得られるのは注\ref{rem:inventory-cut}の
二つの断面をつなぐ情報であり、本章の結論を動かすものではない。

\end{enumerate}

\subsection{優先順位}

実行可能性、波及範囲、費用の三点で評価する。
着手できるものと、労力の投入では前進しないものに分けて示す。
...
\toprule
 & 項目 & 分類 & 実行可能性 & 波及 \\
\midrule
1 & (17) $\lambda_{\rm novel}$ の成分分解（残りは内訳のみ） & 開示選択 & 低 & 中 \\
2 & (3) R2 の $H$ と $\beta$ & (ii) & 中 & 中 \\
3 & (5) $\Delta\ln W$ の変動要因 & (iii) & 中 & 中 \\
4 & (11) 個人事業主の $\kappa$ & 集計の選択 & 中 & 大 \\
5 & (13) 主体単位の $\kappa_i$ & 集計の不能 & 低 & 大 \\
6 & (18) 棚卸資産の範囲（規模との交差） & 集計の選択 & 低 & 小 \\
\bottomrule
\end{tabularx}
\captionof{table}{着手できる項目の優先順位。}
...
\toprule
 & 項目 & 分類 & 前進しない理由 \\
\midrule
7 & (6) $\phi_{\rm cog}$ & (iv) & 個票と外生ショックに到達しない \\
8 & (1) 役員報酬 & (i) & 労働時間の統計が存在しない \\
9 & (2) 可測性仮説 & (ii)＋(vi) & 説明変数が動いていない \\
10 & (7) 余剰の取りやすさ & (iv) & 仲介者は業種の属性でない \\
11 & (4) 売掛金伸長の解釈 & (iii) & 電子記録債権の残高が調査項目にない \\
12 & (8) 年齢効果の曲率 & (iv) & 企業年齢を持つ標本がない \\
13 & (14) 無人化の程度 & --- & 測度が定義されない \\
14 & (15) $\phi$ の成分分離 & --- & $p^{\ast}$ が反実仮想 \\
15 & (10) 集約の社会的最適性 & --- & 対象設定の外 \\
16 & (9) 容量制約 & --- & 同上 \\
17 & (12) 請求権の流通 & --- & 定義域の拡張を要する \\
18 & (16) 第三者の除去の程度 & --- & 同上 \\
\bottomrule
\end{tabularx}
\captionof{table}{労力の投入では前進しない項目。}
\label{tab:unresolved-blocked}
\end{center}

（i）と（ii）に属する項目は労力の投入では前進しない。

このうち(11)個人事業主の $\kappa$ は性質が異なる。
...
(13)手続き上の記録は、第\ref{ch:protocol}章の区分「内」において
本稿が個社データを要するとして断念した量が公開されている項目である。
ただし注\ref{rem:additivity}により主体単位への集計ができないため、
測れるのは識別子単位の分布にとどまる。

\textbf{障害を持たない項目は二件あり、いずれも公表データまたは
既に確立した取得手続きで着手できる。}
(18)は62業種について実施済みで、残るのは規模階層との交差である
（第\ref{sec:inventory-scope}節）。
表\ref{tab:unresolved-blocked}の項目とは、要する作業の種類が異なる。
...
\bottomrule
\end{tabularx}
\captionof{table}{独立性の過剰仮定に由来して外れた4件の含意。}
...
シフト・シェア分解 & 水準変化を業種内効果と構成効果に分解 \\
弾力性推定 & 変化同士の関係として $\beta$ を推定 \\
\bottomrule
\end{tabularx}
...
水準と変化が別の制約であることも、命題\ref{prop:fundamental}の基本不等式を境に整理された。
$M\geq 0$ が水準を、$\dot M\geq 0$ が変化を抑える（注\ref{rem:level-and-change}）。
両者は独立だが、$\tau_i$ に対する感度は同一の係数 $-1/\CCC$ を持つ
（命題\ref{prop:tau-substitution}、表\ref{tab:comparative-statics}）。
...
弱点 & 該当箇所 & 接続しうる理論 \\
\midrule
(A) $\bar\delta$ と $\delta$ & 注\ref{prop:breakage}、族2、フィットネス & メカニズムデザイン、自己選択と数量割引 \\
...
第\ref{sec:predB-illposed}節で含意を立てる際にその論争状況を確認しなかったのは、
同じ理論を参照するときと含意を立てるときで異なる深さで扱っていたためである。
