...
本稿では22の予測を事前に文章として固定した。
支持されたのは6件（支持3件、部分的支持3件）、棄却が11件、検証不能が5件である。
うちDと$\Phi$1 は同一の関係を二度登録したものであり、$\Phi$1 の検定が両方の判定になる。
...
D & 無人化 & 依存の数と無人期間。$\Phi$1 と同一の関係であり同時に棄却 & 棄却 \\
...
\bottomrule
\end{tabularx}
\captionof{table}{事前登録した22予測の対象・内容・判定結果（A--T）。}
\label{tab:predictions-AT}
\end{center}
...
Y & 族5 & 規模と離職率の相関の職種順序。定性的な区分のみ規則によらず成立 & 部分的支持 \\
Z & 族5 & B1 の大きい職種ほど返戻金が強い。順位相関 $\rho=0.000$ & \textbf{棄却} \\
$\Phi$1 & 手続き & 依存の数と改修までの期間。複製の集中が結果を作っていた（Dと同一） & 棄却 \\
...
\bottomrule
\end{tabularx}
\captionof{table}{事前登録した22予測の対象・内容・判定結果（U--$\Phi$4）。}
\label{tab:predictions-22}
\end{center}

Q・R・S は労働者視点の検討過程で登録したが、
第\ref{sec:worker-predictions}節のとおり検証に着手していない。
$\Phi$1--$\Phi$4 は第\ref{sec:protocol-predictions}節で登録し、
第\ref{sec:protocol-results}節で検定した。第\ref{part:catalog}部の演繹に対する初めての検定である。
...
\item 第\ref{sec:protocol-results}節の $\Phi$2 において、
途絶した $\Phi$ の残高が事前に減少していることだけを見れば支持と読めた。
継続した $\Phi$ にも同じ窓を当てると、180 日では両者が分離しない。
\textbf{対照を置くと決めていなければ、全体の減少基調を支持と読み違えた。}

\item 同節の $\Phi$4 において、唯一現れた族7 が
受益者の選定を手続きの外へ出していることは、
命題\ref{prop:beneficiary}の傍証として提示できた。
しかし予測の文言は「残るのは 7-4」であり、現れたのは 7-2 である。
\textbf{機構が命題と整合することは、外れた予測を救わない。}
\end{itemize}
...
第\ref{sec:protocol-results}節のとおり予測$\Phi$1--$\Phi$4 を検定した。
