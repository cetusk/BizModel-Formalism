...
5 & (4) 売掛金伸長の解釈 & (iii) & 低 & 小 \\
6 & (6) $\phi_{\rm cog}$ & (iv) & 低 & 大 \\
7 & (1) 役員報酬 & (i) & 低 & 大 \\
8 & (2) 可測性仮説 & (ii) & 低 & 中 \\
9 & (7) 余剰の取りやすさ & (iv) & 低 & 中 \\
10 & (9) 集約の社会的最適性 & --- & 低 & 中 \\
...
\bottomrule
\end{tabularx}
\captionof{table}{未解決問題12項目の分類・実行可能性・波及範囲による優先順位。}
...
残る一つが(12)手続き上の記録である。
第\ref{ch:protocol}章の区分「内」では、本稿が個社データを要するとして
断念した量が公開されており、\textbf{取得の障害がない点で他の11項目と異なる}。
ただし注\ref{rem:additivity}により主体単位への集計ができないため、
測れるのは識別子単位の分布にとどまる。
何が言えるかは取得してみなければ分からない。

\textbf{公表データの取得のみで前進できる項目は、この一つを除いて残っていない。}
