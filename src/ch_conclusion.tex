...
前項(11)と同じ箇所、すなわち添字の設定に由来する制約である。
...
\item \textbf{標本の地理的拡張。}
第\ref{sec:predX-result}節の307社は東京都に限られる。
...
\toprule
 & 項目 & 分類 & 実行可能性 & 波及 \\
\midrule
1 & (17) 標本の地理的拡張 & 障害なし & 高 & 中 \\
2 & (18) 予測Zの検定 & 障害なし & 中 & 大 \\
3 & (20) 棚卸資産の範囲（45業種へ拡張） & 障害なし & 高 & 中 \\
4 & (12) 予測$\Phi$1--$\Phi$4 の検定 & 集計の不能 & 高 & 大 \\
5 & (19) 返戻金の率と期間 & 障害なし & 中 & 中 \\
6 & (21) 年齢効果の曲率 & 障害なし & 中 & 小 \\
7 & (16) $\lambda_{\rm novel}$ の成分分解 & 障害なし & 中 & 中 \\
8 & (3) R2 の $H$ と $\beta$ & (ii) & 中 & 中 \\
9 & (5) $\Delta\ln W$ の変動要因 & (iii) & 中 & 中 \\
10 & (10) 個人事業主の $\kappa$ & 集計の選択 & 中 & 大 \\
\bottomrule
\end{tabularx}
...
\toprule
 & 項目 & 分類 & 前進しない理由 \\
\midrule
11 & (6) $\phi_{\rm cog}$ & (iv) & 個票と外生ショックに到達しない \\
12 & (1) 役員報酬 & (i) & 労働時間の統計が存在しない \\
13 & (2) 可測性仮説 & (ii)＋(vi) & 説明変数が動いていない \\
14 & (7) 余剰の取りやすさ & (iv) & 仲介者は業種の属性でない \\
15 & (4) 売掛金伸長の解釈 & (iii) & 電子記録債権の残高が調査項目にない \\
16 & (13) 無人化の程度 & --- & 測度が定義されない \\
17 & (14) $\phi$ の成分分離 & --- & $p^{\ast}$ が反実仮想 \\
18 & (9) 集約の社会的最適性 & --- & 対象設定の外 \\
19 & (8) 容量制約 & --- & 同上 \\
20 & (11) 請求権の流通 & --- & 定義域の拡張を要する \\
21 & (15) 第三者の除去の程度 & --- & 同上 \\
\bottomrule
\end{tabularx}
...
(12)手続き上の記録は、第\ref{ch:protocol}章の区分「内」において
本稿が個社データを要するとして断念した量が公開されている項目である。
ただし注\ref{rem:additivity}により主体単位への集計ができないため、
測れるのは識別子単位の分布にとどまる。

\textbf{障害を持たない項目は六件あり、いずれも公表データまたは
既に確立した取得手続きで着手できる。}
うち(20)は10業種について実施し、残るのは全業種への拡張である
（第\ref{sec:inventory-scope}節）。
表\ref{tab:unresolved-blocked}の11項目とは、要する作業の種類が異なる。
