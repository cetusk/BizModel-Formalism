...
支持されたのは5件（支持2件、部分的支持3件）、棄却が7件、検証不能が4件である。
...
\item \textbf{予測YとZの検定。}
規模と離職率の相関を B1 の代理と読み、
第\ref{sec:b1-proxy}節で二つの予測を登録した。
予測Zが成立すれば、符号が一意に定まらないとした予測Bについて
B1とB2を分離する経路が得られる。
ただし代理として用いうるだけであり、
投入の希薄化と選別の緩みは区別できない（注\ref{rem:b1-proxy-assumption}）。

\textbf{同一標本で検定してはならない}（注\ref{rem:no-same-sample-test}）。
相関は307社において事後に観測されたものであり、
要求されるのは標本外での再現である。他都道府県の取得手続きは同一である。
