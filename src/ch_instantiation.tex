...
到達段階は量ごとに異なる（注\ref{rem:stage-per-quantity}）。
本章の一覧は、第\ref{sec:five-stages}節の段階1から3のどこで詰まったかの記録にあたる。
...
長期の時系列で $m/\CCC$ を用いる理由は表\ref{tab:gstar-measurement-stages}に述べた。
...
\textbf{この含意は支持される。} 個人企業の営業利益率は6年間すべてで法人の5倍前後であり、
...
式\eqref{eq:cog}の $\phi_{\rm cog}=p\cdot\E[\dbar-\delta]$ は測定可能な量であり、
測定例もある。以下に四つを挙げる。
\textbf{ただし本稿が自ら得たのは上界のみで、値は未知である。}

\subsection{\texorpdfstring{$\phi_{\rm cog}$}{phi-cog} は測定可能である}
\label{sec:phicog-measurable}

\paragraph{本稿の測定：族2の有効期限到来等による回収額。}
...
\textbf{得られたのは上界であって $\phi_{\rm cog}$ そのものではない。}
以下の三例は、いずれも値そのものを与えている。

\paragraph{フィットネスクラブの会員契約。}
...
\cite{nber2026pnbl} は族2-4（プリペイド）を直接扱う。
...
式\eqref{eq:delta-star}は $p\,\delta^{*}=F$ と同値であるから、
これを式\eqref{eq:cog}に代入すると
\begin{equation}
\phi_{\mathrm{cog}} \approx p\,\E[\delta^{*}-\delta] = F - p\,\E[\delta]
\label{eq:cog-menu}
\end{equation}
となる。\textbf{この代理の下で認知余剰は、定額料金と従量換算額の差である。}
登録した含意 $\delta^{*}-\delta>0$ は、式\eqref{eq:cog-menu}が正であること、
すなわち定額料金が平均的な利用の従量換算を上回ることと同じ主張になる。

\begin{quote}
\textbf{登録した含意}：定額会員について $\delta^{*} - \delta > 0$ である。
...
\textbf{この含意は棄却される。} 四カテゴリすべてで符号が負であり（表\ref{tab:predk-breakeven}）、
...
第\ref{sec:obstacle-taxonomy}節の分類で言えば、
