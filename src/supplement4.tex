...
応答を状態空間上の可測関数 $a:\Omega\to\mathcal{A}$ として\emph{事前に}書き下すこと。
$\mathcal{A}$ は取りうる応答の集合である（固定資産 $A$ とは別の量であり、書体で区別する）。
\end{definition}
...
\begin{remark}[$\lambda_{\rm novel}$ の上限が測れる]
\label{rem:lambda-upper}
第\ref{ch:protocol}章の区分「内」では、未想定状態への\emph{応答}が記録に残る。
改修が 0.048、停止が 0.030（いずれも毎年）であり、
応答を要さなかった到来は数えられないから、これらは $\lambda_{\rm novel}$ の下限である
（第\ref{sec:protocol-byproducts}節、注\ref{rem:lambda-is-response}）。
したがって本命題の $1/\lambda_{\rm novel}$ は 20 年より短い。

分布は二峰である。手を入れるものは中央値 42 日で入れ、
入れないものは中央値 4.6 年にわたり入れない。
\textbf{平均としての $1/\lambda$ はこの二峰性を覆い隠す。}
\end{remark}
...
両者を同時に課すと、負担の内容がより具体的に書ける。

\begin{proposition}[可用性は容量の粒度を制限する]
\label{prop:granularity}
式\eqref{eq:availability}の下では、履行に充てられる時間は
長さ $T_{\rm resp}$ 以下の区間に分断される。
したがって連続した $T_{\rm resp}$ 超の時間を要する履行は、
式\eqref{eq:solo-capacity}の $\mathrm{Cap}$ に余裕があっても実行できない。
\end{proposition}

\begin{proof}
$\sup u\leq T_{\rm resp}$ は、応答しない区間の長さが $T_{\rm resp}$ を超えないことを要求する。
履行に専念している区間は応答しない区間であるから、その長さも $T_{\rm resp}$ 以下である。
連続した $T_{\rm resp}$ 超の時間を要する履行はこの区間に収まらない。
\end{proof}

\textbf{可用性制約は容量を減らすのではなく、容量の粒度を制限する。}
第\ref{ch:solo}章の $T_{\rm dev}$ は長い連続時間を要する一方、
運用は短い応答の反復である。命題\ref{prop:granularity}は、
一人事業において開発と運用が両立しにくいことを、
二つの制約の交差として与える。

\begin{remark}[緩和の方向が異なる]
式\eqref{eq:solo-capacity}は人を増やせば緩む。
式\eqref{eq:availability}は交代要員を置かない限り緩まず、
一人である限り $T_{\rm resp}$ そのものを延ばすほかない。
応答期限の延長は、契約上の合意か、$\Omega\setminus\hat\Omega$ の縮小によって得られる。
\end{remark}
...
$\Phi$ と $C_{\mathrm{prod}}$ が完全に事前指定できるならば、それは完全に複製可能である。
