...
第\ref{ch:protocol}章の区分「内」では、$M(t)$、$\kappa_j$、$\dbar-\delta$、$B$ が
識別子ごとに公開されている（第\ref{sec:protocol-observability}節）。
他の章では個社データを要するため測れない量である。本章はこれを観測する。
...
第\ref{ch:next}節の手続きに従い、取得の前に四つの含意を固定した。
区別のため $\Phi$ に番号を付す。

\begin{quote}
\textbf{含意$\Phi$1}：手続きが呼び出す他の手続きの数が少ないほど、
...
\begin{quote}
\textbf{含意$\Phi$2}：取引が途絶えた $\Phi$ について、
$M(t)$ は途絶に先行して単調に減少する。
...
\begin{quote}
\textbf{含意$\Phi$3}：区分「内」で稼働する $\Phi$ の $\CCC$ の分布は非負に偏る。
...
\begin{quote}
\textbf{含意$\Phi$4}：稼働する $\Phi$ を第\ref{sec:type-assignment}節の規則で割り当てると、
...
\bottomrule
\end{tabularx}
\captionof{table}{四つの含意の検定結果。}
\label{tab:protocol-results}
\end{center}

\begin{remark}[複製の集中が推定を支配する]
...
含意の検定に用いた記録から、他の章で測れなかった量が三つ測れる。
