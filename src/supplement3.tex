...
$\tau$ & 第\ref{sec:objective}章 & 期限として $E(0)/c$ で上から抑えられる \\
...
第\ref{sec:objective}章の経路制約により毎時点で成立する必要がある。
...
第\ref{part:catalog}部の27類型を、式\eqref{eq:solo-finance}と\eqref{eq:solo-capacity}で篩にかける。
...
これは第\ref{sec:objective}章の経路制約とも整合する。大口一社への依存は、
その一社の遅延で $\tau$ に到達する。
...
第\ref{sec:objective}章の定式化に翻訳すると、変化するのは $H$ と $\beta$ である。
...
第\ref{sec:objective}章の枠組みがそのまま使える。
...
第\ref{sec:objective}章の制約を $M(t)\geq 0$ と書いてきたが、この層には不適切である。
...
これは第\ref{sec:objective}章の割引因子の議論と同型である。
