...
$\Tins$ & 第\ref{sec:objective}節 & 期限として $E(0)/c_{\rm liv}$ で上から抑えられる \\
...
これは系\ref{cor:zero-equity}の条件であり、$\CCC\leq 0$、
すなわち $\sum_i\kappa_i\leq 0$ が要求される。総額の形で書けば
...
である。式\eqref{eq:solo-finance}は、\textbf{引き出した与信が与えた与信を常に上回っていなければならない}
ことを述べている。これは仮定ではなく、式\eqref{eq:fundamental}の基本不等式に
$E=A=0$ を代入した特例である。命題\ref{prop:substitution}の代替関係が
$E=0$ の端点で現れた形でもある。
...
\begin{equation}
\sum_{i} \delta_i(t) \;\leq\; \mathrm{Cap}, \qquad \mathrm{Cap} = \text{一人分の時間}
...
\subsection{どちらが拘束するか}
\label{sec:which-binds}

式\eqref{eq:solo-finance}と式\eqref{eq:solo-capacity}は別々に導入したが、
同時に課される。両者を規模 $r$ についての上限に直すと比較できる。

\begin{proposition}[拘束の入れ替わり]
\label{prop:binding-switch}
系\ref{cor:rmax}より、$\CCC>0$ のとき現金制約は $r\leq(E-A)/\CCC$ を与える。
式\eqref{eq:solo-capacity}を価値に直すと容量制約は $r\leq v(\mathrm{Cap})$ を与える。
したがって
\begin{equation}
\CCC^{\dagger} = \frac{E-A}{v(\mathrm{Cap})}
\label{eq:ccc-dagger}
\end{equation}
を境として、式\eqref{eq:ccc-dagger}の $\CCC>\CCC^{\dagger}$ では現金制約が、
$\CCC<\CCC^{\dagger}$ では容量制約が拘束する。
\end{proposition}

\begin{proof}
二つの上限のうち小さいほうが拘束する。
$(E-A)/\CCC \leq v(\mathrm{Cap})$ となるのは $\CCC\geq(E-A)/v(\mathrm{Cap})$ のときである。
\end{proof}

\textbf{$\CCC$ を下げる努力が規模を増やすのは $\CCC>\CCC^{\dagger}$ の範囲だけである。}
それより下では容量が拘束しており、契約条件を変えても $r$ は増えない。

$E\approx 0$ の一人事業では $\CCC^{\dagger}\approx 0$ である。
第\ref{part:theory}部で企業を扱った際は財務制約 $M(t,\omega)\geq 0$ が拘束したが、
ここでは $\CCC$ を負にした時点で拘束が容量へ移る。
顧客が増えたとき、現金より先に時間が尽きる。
\textbf{本章が前受型を選んだ先で容量制約に突き当たるのは、この移動による。}
...
月間の生活費とインフラ費の速度を $c_{\rm liv}$ として、
...
\begin{align}
\text{給与所得者：}\quad & E(t) = E(0) + (w-c_{\rm liv})\,t \qquad (w>c_{\rm liv} \text{ なら増加}) \\
\text{退職者：}\quad & E(t) = E(0) - c_{\rm liv}\,t \qquad\qquad\ \ (\text{単調減少})
\end{align}

給与所得者にとって式\eqref{eq:constraint}の $M\geq 0$ は\textbf{床}である。
近づけば引けばよく、時間は味方となる。
...
安全な経路の期待所要時間が $E(0)/c_{\rm liv}$ を超える場合、その経路の成功確率はほぼゼロである。
\end{proposition}
...
年金 & 生活費 $c_{\rm liv}$ を事業の外へ切り出す & 目的関数 $\to$ 外生 \\
\bottomrule
\end{tabularx}
...
これは命題\ref{prop:substitution}の代替関係を、
本章の $E\approx 0$ という端点で見たものである。
...
経験は $C_{\mathrm{prod}}$ を下げるが、$\kappa$ の符号を動かすのは信用である。
...
\subsection{部分変換と最適な配分}

右上を目指す場合でも、代替は完全な一対一ではない。
式\eqref{eq:solo-capacity}の $\mathrm{Cap}$ のうち受託に充てる分を
$\Ccl$、公開に充てる分を $\mathrm{Cap}-\Ccl$ とすると、

\begin{equation}
\text{公開型信用の蓄積} = \gamma\,(\mathrm{Cap}-\Ccl) + \theta\,\Ccl,
\qquad 0<\theta<\gamma
\label{eq:leakage}
\end{equation}
...
式\eqref{eq:leakage}は $\Ccl$ について線形であり、$0<\theta<\gamma$ より狭義単調減少である。
これだけでは最適な配分は $\Ccl=0$、すなわち受託をまったく行わないことになる。
配分が内点で決まるのは、受託が現金をもたらすからである。

\begin{proposition}[受託は制約が要求する最小限にとどまる]
\label{prop:allocation}
受託の時間あたり報酬を $w$、生活費の速度を $c_{\rm liv}$ とする。
式\eqref{eq:fundamental}の基本不等式が要求する現金は $w\,\Ccl\geq c_{\rm liv}$ であり、
式\eqref{eq:leakage}を最大化する配分は
\begin{equation}
...
\begin{proof}
目的は $\Ccl$ の狭義減少関数であり、実行可能集合は $\Ccl\geq c_{\rm liv}/w$ である。
したがって最小の実行可能な $\Ccl$ が最適となる。
\end{proof}

\textbf{最適解は目的関数の内点ではなく、制約の縁である。}
$E(0)>0$ であれば制約が $w\,\Ccl\geq c_{\rm liv} - E(0)/T$ に緩み、$\Ccl^\ast$ は下がる。
第\ref{sec:retiree}節の「資本は信用を積む期間を買う」は、
式\eqref{eq:allocation}の右辺が下がることとして書ける。

配分そのものとは別に、$\theta$ を上げる余地がある。
...
これは第\ref{sec:objective}節の割引因子の議論と同型であり、
第\ref{ch:catalog}章で族5-1 と 1-3 の選択について述べたのと同じ構造である。
