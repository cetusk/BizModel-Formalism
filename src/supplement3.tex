...
$\Tins$ & 第\ref{sec:objective}節 & 期限として $E(0)/c$ で上から抑えられる \\
...
式\eqref{eq:solo-capacity}の $\mathrm{Cap}$ を
$a$（受託）と $\mathrm{Cap}-a$（公開）に配分すると、
...
式\eqref{eq:leakage}は $a$ について線形であり、$0<\theta<\gamma$ より狭義単調減少である。
これだけでは最適な配分は $a=0$、すなわち受託をまったく行わないことになる。
配分が内点で決まるのは、受託が現金をもたらすからである。

\begin{proposition}[受託は制約が要求する最小限にとどまる]
\label{prop:allocation}
受託の時間あたり報酬を $w$、生活費の速度を $c$ とする。
式\eqref{eq:fundamental}の基本不等式が要求する現金は $w\,a\geq c$ であり、
...
\begin{proof}
目的は $a$ の狭義減少関数であり、実行可能集合は $a\geq c/w$ である。
したがって最小の実行可能な $a$ が最適となる。
\end{proof}

\textbf{最適解は目的関数の内点ではなく、制約の縁である。}
$E(0)>0$ であれば制約が $w\,a\geq c - E(0)/T$ に緩み、$a^\ast$ は下がる。
