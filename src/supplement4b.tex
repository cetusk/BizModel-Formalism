...
直接検証できる。第\ref{part:method}部の規律に従い、調査前に予測を確定した。

\begin{quote}
\textbf{予測C}：プラットフォームの手数料率は、
...
本調査は第\ref{sec:obstacle-taxonomy}節の四分類において
...
式\eqref{eq:tacq-price}は予測Cの最も強い証拠である。
...
\subsection{リスク成分についての未検証の予測}
\label{sec:pred-J}

式\eqref{eq:fee-decomp3}の第二項について、
命題\ref{prop:pooling}から予測が導かれる。
...
本予測の効果はここで述べるより強く現れる。
\textbf{仮定を外しても予測の符号は変わらない。}

\begin{quote}
\textbf{予測J}：決済代行の業種別料率の差は、
...
これは第\ref{sec:layers}章の耐久性の主張とも整合する。
...
第\ref{sec:layers}章で、支配力に由来する $\phi_{\rm barg}$ は制度層に削られると述べた。
本節はその作用形態を四例で確認する。
...
後者が近年の主流である。
第\ref{sec:layers}章の記述はこの区別を含んでいないため、
「上限を課す」を「制約を課す」と読むべきである。
\end{remark}

\section{調査可能性の判定}

本章の他の論点について、第\ref{sec:obstacle-taxonomy}節の四分類を適用した結果を記録する。
...
$\lambda_{\rm novel}$ は分解でき、成分が第\ref{sec:layers}章のレイヤーに対応する。
...
四成分のうち三つに公開データが存在する。予測が立つ。

\begin{quote}
\textbf{予測D}：依存の少ない構成ほど $\lambda_{\rm dep}$ が小さく、無人運転可能期間が長い。
\end{quote}
...
それが存在しない。第\ref{sec:predB-illposed}節の予測Bと同型の欠陥である。
...
\item 可用性の消費量を定式化していない。
「拘束を消費する」と述べたが、時間と異なり単位を与えていないため、
容量制約のような不等式として書けていない。

\item 第\ref{sec:platform-fee}章は母集団を宣言していない事例比較である。
同一プラットフォーム内の識別を除き、推定として扱ってはならない。

\item 無人化と利潤の逆相関は機構の指摘にとどまる。
