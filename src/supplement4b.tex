...
同じ並びを図\ref{fig:platform-ladder}に示す。
...
注\ref{rem:independence-finite}のとおり式\eqref{eq:pooling}の極限は $\rho\sigma^2$ で下げ止まる。
...
本節の四例はいずれも料率に関わるため、注\ref{rem:institution-modes}の三形式のうち
価格規定と競争条件の操作の二つに当たり、参入要件は現れない。
\textbf{近年の主流は後者である。}
料率を直接規定する形式は減少し、競争条件の操作に置き換わっている。
\end{remark}
