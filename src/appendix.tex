...
\label{prop:apc}
本命題は Age--Period--Cohort 文献の記法に従う。
$a,p,c,\pi,y$ は本節に限り年齢・時点・コホート・時点効果・観測値を表し、
本稿の他の箇所での用法（受託配分・単価・限界費用・決済・成果）とは無関係である。
...
後者は期限制約 $\Tins \leq E(0)/c_{\rm liv}$ の下での結論である。
...
これは第\ref{sec:objective}節の $\E[\sum\beta^t\phi_t]$ とは異なる。
...
第\ref{sec:objective}節の倒産時刻 $\tau$ も吸収状態であり同じ性質を持つ。
...
\caption{記号一覧：基本量と写像。}\label{tab:notation-basic}\\
\toprule
記号 & 名称 & 定義または初出 \\
\midrule
\endfirsthead
...
\endhead
\midrule
\multicolumn{3}{r}{\footnotesize（次頁へ続く）}\\
\endfoot
\bottomrule
\endlastfoot
...
$W$ & 運転資本 $\sum_i\kappa_i+\mathrm{Inv}$ & 定義\ref{def:wc} \\
$M(t)$ & 現金残高。$M=E-W-A$ & 式\eqref{eq:cash}、命題\ref{prop:cash-net} \\
$A,\ \mathrm{Inv}$ & 固定資産、棚卸資産 & 命題\ref{prop:cash} \\
$\Phi$ & ビジネスモデル & 式\eqref{eq:phi} \\
$C(\delta),\ c(\delta)$ & 総費用、限界費用。$c$ はつねに引数を伴う & 定義\ref{def:cost} \\
$F,\ p$ & 料金の固定部分、単価 & 式\eqref{eq:two-part} \\
$h,\ y$ & 成果連動の写像、成果 & 同上 \\
$E$ & 自己資本。払込資本＋内部留保 & 式\eqref{eq:equity} \\
$\kappa_K^{i}$ & 資本提供者との信用ポジション。主体依存 & 定義\ref{def:kappa-K} \\
$V_i$ & 資本提供者 $i$ の測度による残余請求権の価値 & 同上 \\
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.32\textwidth}>{\raggedright\arraybackslash}p{0.34\textwidth}@{}}
\caption{記号一覧：派生量と方法論の記号。}\label{tab:notation-derived}\\
\toprule
記号 & 名称 & 定義または初出 \\
\midrule
\endfirsthead
...
$\Tins$ & 倒産時刻。$\tau_i$ と区別する & 命題\ref{prop:pathwise}、式\eqref{eq:depletion} \\
$\beta_i$ & 主体 $i$ の割引因子 & 第\ref{sec:objective}節 \\
$r,\ m$ & 売上速度、粗利率・営業利益率 & 第\ref{sec:growth}章 \\
$\CCC$ & キャッシュコンバージョンサイクル & 式\eqref{eq:ccc} \\
$r^{\max}$ & 規模の上限 $(E-A)/\CCC$ & 式\eqref{eq:rmax} \\
...
$a,p,c$ & 年齢、時点、コホート。本節に限る & 式\eqref{eq:apc-main} \\
$\mathrm{Cap}$ & 容量上限 & 式\eqref{eq:solo-capacity} \\
$\Ccl$ & 容量のうち受託に充てる分 & 式\eqref{eq:leakage} \\
$c_{\rm liv}$ & 生活費の速度 & 式\eqref{eq:E-required-2} \\
$\mathcal{A}$ & 応答の集合。固定資産 $A$ と区別する & 定義\ref{def:availability} \\
$T_{\rm resp},\ u$ & 応答期限、応答できない区間の長さ & 式\eqref{eq:availability} \\
...
\caption{既存理論との対応、第三層(a)：個別の対応関係。}\label{tab:priorwork-layer3}\\
\toprule
本稿の内容 & 対応する既存理論 \\
\midrule
\endfirsthead
...
\endhead
\midrule
\multicolumn{2}{r}{\footnotesize（次頁へ続く）}\\
\endfoot
\bottomrule
\endlastfoot
...
式\eqref{eq:gstar} の $g^\star$ & 自己金融可能成長率 \cite{churchill2001}。配当性向と負債比率を固定する Higgins 系の持続可能成長率とは別系統 \\
式\eqref{eq:cash-net}（$M=E-W-A$） & 財務分析の標準的な恒等式。現金 ＝ 自己資本 $-$ 正味営業資産 \\
命題\ref{prop:fundamental}、系\ref{cor:rmax}（規模の上限） & 過大取引（overtrading）の形式化。運転資本が純資産で抑えられる構造は \cite{kiyotaki1997} と同型 \\
命題\ref{prop:cog-cash-neutral}（認知余剰の現金中立性） & 収益認識における契約負債からの振替。会計上の標準的な扱い \\
命題\ref{prop:peak}（尖頭と平均の差） & 実務でいう資金需要のピーク。運転資本管理の標準的な論点 \\
系\ref{cor:ccc-weighted}（$\CCC$ の非加法性） & 比の集計一般の性質。式\eqref{eq:shift-share}のシフト・シェア分解はその差分形 \\
...
命題\ref{prop:substitution}（資本と信用の代替） & 担保制約と純資産 \cite{kiyotaki1997,rampini2010,rampini2013} \\
信用のブートストラップ問題（第\ref{sec:bootstrap}節） & 同上。純資産の低い主体ほど制約が強いという結果に対応 \\
手続きによる契約空間の拡大（第\ref{ch:protocol}章の前提(1)(2)） & 分散合意と契約可能性 \cite{cong2019} \\
識別子と主体の乖離（第\ref{sec:identifier}節） & Sybil 攻撃 \cite{douceur2002} \\
系\ref{cor:exclusion}（資産を持たない主体の排除） & 担保要求と金融包摂の限界 \cite{oecd2024} \\
注\ref{rem:oracle}（区分「外」における仲介） & 外部状態の取り込みとその検証可能性 \cite{pasdar2023} \\
命題\ref{prop:beneficiary}（受益者の同定） & 識別子の自由な生成と false-name-proofness \cite{yokoo2004} \\
\end{longtable}

\begin{center}
\begin{tabularx}{\textwidth}{@{}XX@{}}
\toprule
接続すべきだが接続していない理論 & 対応する既存理論 \\
\midrule
...
規模と紹介の質の関係（第\ref{sec:b1-proxy}節） & 労働市場の仲介と適合の質 \\
\bottomrule
\end{tabularx}
\captionof{table}{既存理論との対応、第三層(b)：接続すべきだが接続していない理論。}
...
レイヤー構造（第\ref{sec:layers}節） & 本稿の整理 \\
$\Phi$ の類型カタログ（第\ref{part:catalog}部） & 本稿の整理 \\
検証障害の六分類（第\ref{sec:obstacle-taxonomy}節） & 本稿の整理 \\
\bottomrule
\end{tabularx}
\captionof{table}{既存理論との対応、第三層(c)：本稿固有の整理。}
\label{tab:priorwork-layer3c}
\end{center}

\input{app_prereg.tex}
\input{app_revisions.tex}
...
族5 & 人材サービス総合サイト、事業所詳細と返戻金の届出書類 & 第\ref{sec:predZ-result}節 \\
...
\paragraph{再現について。}
取得したデータと処理を \texttt{data/} に置いた。
\texttt{raw/} が取得したままの入力、\texttt{scripts/} が分析の処理である。
\texttt{derived/} は中間データであり、\texttt{scripts/parse.py} で再生成する。

第\ref{part:industry}章の算出は \texttt{ccc.py}、\texttt{e1.py}、\texttt{elast.py} ほか、
第\ref{sec:predY-result}節の検定は \texttt{parse\_jinzai.py} と \texttt{predY.py} による。
対応の一覧は \texttt{data/README.md} にある。

\paragraph{取り扱いについて。}
用いたのはすべて公開統計である。ただし人材サービス総合サイトだけは、
集計値ではなく\textbf{事業所ごとの届出内容}を個別に掲載しており、
本稿はこれを統計解析の入力として用いている。

\textbf{本稿が示すのは解析の結果であって、収集した個社データの集合ではない。}
それを複製して公開・再配布することはしない。
リポジトリに含めるのは分析の処理と、そこから得た集計結果である。
同サイトから取得した個社データは、検索結果の一覧も事業所詳細のページも
返戻金制度の届出書類も含めない。

残してあるのは\textbf{許可・届出受理番号の一覧}（\texttt{data/raw/jinzai/permits.tsv}）
だけで、届出の値は含まない。これを種として
\texttt{data/scripts/fetch\_jinzai\_detail.py} を回せば、
手数料実績率・離職率・就職者数は再現できる。
取得は1件あたり6秒の間隔を置き、日を分けて行った。
